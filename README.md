...
Continuuuum library server: upload, search, and view documents with location and type-specific metadata. Uses **USC** (unified-semantic-compressor; Python package name `unified_semantic_archiver`) for the continuuuum DB and schema.
...
| **continuuuum** (this repo) | Library web UI + Flask API (upload, search, media parity) | [README.md](README.md) |
...
Unity ↔ Python bridge: [../Drawer 2/Scripts/CONTINUUUUM_AND_COMPRESSOR.md](../Drawer%202/Scripts/CONTINUUUUM_AND_COMPRESSOR.md)
...
| **Media parity API** | Same server | Store, reconstitute, stream, T2V — see [Media Parity](#media-parity-usc--continuuuum) |
...
- [docs/USC_CONTINUUUUM_MEDIA_INTERFACE_CONTRACT.md](docs/USC_CONTINUUUUM_MEDIA_INTERFACE_CONTRACT.md) — Media interface contract
...
- [library/continuuuum_editor_webgl/README.md](library/continuuuum_editor_webgl/README.md) — WebGL editor stub

### Sibling repositories

- [Drawer 2 documentation index](../Drawer%202/README.md#documentation-index)
- [Unity ↔ Python bridge](../Drawer%202/Scripts/CONTINUUUUM_AND_COMPRESSOR.md)
...
2. **Install continuuuum deps:** from this repo, `pip install -r requirements.txt`
3. **Init DB:** `python -m unified_semantic_archiver init --db ./continuuuum.db`
4. **Run continuuuum server:** `python serve_library.py` → http://localhost:5050
5. **(Optional) Unity (Drawer 2):** Open the project, go to Window → Continuuuum → Continuuuum Library, set Base URL to http://localhost:5050 (and DB path for Explorer). See your Drawer 2 repo’s `Scripts/CONTINUUUUM_AND_COMPRESSOR.md`.
6. **(Optional) Cave (log-view-machine):** Set `CONTINUUUUM_LIBRARY_URL=http://localhost:5050` so the Cave server proxies `/api/continuuuum/library/*` to continuuuum.

## Install

```bash
# From continuuuum repo root; install the compressor first (sibling or clone)
pip install -e ../unified-semantic-compressor
pip install -r requirements.txt
```

**Conda:** For multiple Python versions (e.g. continuuuum + USC with torch), see [docs/CONDA_BUILD_MANAGEMENT.md](docs/CONDA_BUILD_MANAGEMENT.md). Use `conda env create -f conda-environment.yml` then `pip install -e /path/to/unified-semantic-compressor`.
...
- `PORT` — default 5050
- `CONTINUUUUM_DB_PATH` — path to continuuuum.db (default: ./continuuuum.db in repo root)
- `CONTINUUUUM_LIBRARY_UPLOADS` — uploads directory (default: ./library_uploads)
- `FLASK_DEBUG=1` — enable debug mode
- `CONTINUUUUM_API_KEY` — if set, require this value in `X-API-Key` or `api_key` for `/api/library/*` (see [SECURITY.md](SECURITY.md))

## Schema

Continuuuum does not define its own schema. All tables (including `library_documents`) live in **USC** (`unified_semantic_archiver/db/schema.sql`). The continuuuum app uses USC’s `ContinuuuumDb` and the same DB file. To add or change tables, extend USC’s schema and migrations; see the USC repo’s `unified_semantic_archiver/db/SCHEMA_OWNERSHIP.md`.

## First run

Initialize the continuuuum DB (creates schema including library_documents):

```bash
python -m unified_semantic_archiver init --db ./continuuuum.db
```

## Tests

From the continuuuum repo root, run: `pytest tests/` (requires `pip install pytest`). Smoke tests cover search (200) and upload-then-fetch.
...
```bash
python -m entropy.entropythief_node --continuuuum-url http://localhost:5050 --probe-target 8.8.8.8:53
```
...
## Media Parity (USC + Continuuuum)

To retire `video_storage_tool`, Continuuuum tracks feature parity in `library/media_parity_matrix.json`. Contract and retirement docs are in the [Documentation index](#documentation-index) under **Media / USC contract**.
...
Media parity endpoints exposed by Continuuuum:
...
Point the Continuuuum Library window (Window → Continuuuum → Continuuuum Library) Base URL to this server (e.g. http://localhost:5050). The Continuuuum Explorer can use the same continuuuum.db path with the Python CLI.
