#!/usr/bin/env python3
"""
Entropythief petal compute node. Probes target, registers with continuuuum,
submits guesses, participates in ring pass-along.
Run: python -m entropy.entropythief_node --continuuuum-url http://localhost:5050 --probe-target 8.8.8.8:53
"""
...
# Ensure continuuuum repo root on path (parent of entropy/)
...
def main() -> int:
    ...
    parser.add_argument("--continuuuum-url", default="http://localhost:5050", help="Continuuuum server URL")
    ...
    thread.start()
    continuuuum_url = args.continuuuum_url.rstrip("/")
    ...
    while running:
        try:
            if node_id is None:
                ...
                resp = _http_json(f"{continuuuum_url}/api/entropy/nodes/register", "POST", body)
            ...
            center_resp = _http_json(f"{continuuuum_url}/api/entropy/center")
            ...
            _http_json(f"{continuuuum_url}/api/entropy/guess", "POST", {
                "node_id": node_id,
