...
class CaveAdapter:
    ...
    def __init__(self, continuuuum_url: str, credit_ledger: CreditLedger | None = None, db=None):
        self.continuuuum_url = continuuuum_url.rstrip("/")
    ...
    def start_node(
        ...
        *,
        ...
        """Spawn an entropythief petal node. Returns process handle (pid as string). Node registers itself with continuuuum."""
        import sys
        cmd = [
            sys.executable,
            "-m",
    ...
    def list_nodes(self) -> list[dict[str, Any]]:
        """List active and warehouse nodes from continuuuum."""
        resp = _http_json(f"{self.continuuuum_url}/api/entropy/nodes")
    ...
    def tombstone_node(self, node_id: str) -> None:
        ...
        _http_json(
            f"{self.continuuuum_url}/api/entropy/nodes/tombstone",
            "POST",
            {"node_id": node_id},
        )
    ...
    def mezz_node(self, node_id: str) -> None:
        ...
        _http_json(
            f"{self.continuuuum_url}/api/entropy/nodes/mezz",
            "POST",
            {"node_id": node_id},
        )
    ...
    def request_random(
        ...
        try:
            resp = _http_json(
                f"{self.continuuuum_url}/api/entropy/random",
                "POST",
    ...
    def get_credits(self, tenant_id: str = "default") -> dict[str, Any]:
        ...
        return _http_json(f"{self.continuuuum_url}/api/entropy/credits?tenant={tenant_id}")
