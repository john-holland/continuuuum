# Continuuuum Single-Server Architecture
...
**Current phase:** cross-link configuration is documented here; route migration from Drawer 2 `continuuuum_api` is a follow-up PR.
...
| `continuuuumLibraryBase` | [lemma-library.js](../Drawer%202/Scripts/continuuuum_api/static/lemma-library/lemma-library.js) `?libraryBase=`, `localStorage.continuuuumLibraryBase` | `/library` or `/` |
| Unity API | [continuuuum_api_url.txt](../Drawer%202/Scripts/continuuuum_api_url.txt) | Single base URL |
| Cave adapter | [cave_adapter.py](../Drawer%202/Scripts/continuuuum_api/cave_adapter.py) `CAVE_BASE_URL=http://localhost:3000` | `/api/library/*` on merged server |
...
- `PORT=5050`
- `CONTINUUUUM_DB_PATH` — shared `continuuuum.db` (Drawer 2 episodic + USC schemas)
- `CONTINUUUUM_API_KEY` — optional library API auth
- `CONTINUUUUM_DEEPLINK_PATH` — Unity deeplink file (Drawer 2 `continuuuum_api` today)
- `RESAURCE_CAVE_URL` — resaurce Cave for production budget/schedule + chat proxy (default `http://127.0.0.1:3456`)

**Agile PM (Drawer 2 `continuuuum_api` today):**

- `/story-board`, `/project-calendar` — static SPAs with `continuuuum-nav` Chat toggle
...
- `CONTINUUUUM_LEMMA_API_BASE` — lemma library when library SPA runs on a different host
- `CONTINUUUUM_LIBRARY_BASE` — USC library when lemma SPA runs on a different host

---

## Port collision

Both `serve_library.py` (continuuuum repo) and `python -m continuuuum_api.server` (Drawer 2) default to **5050**. Run only one on that port, or set `PORT` / `--port` on the other.

| Service | Default | Notes |
|---------|---------|-------|
| serve_library | 5050 | Geo + spatial map, `/api/library/*`, `/api/spatial/*` |
| continuuuum_api | 5050 | Lemma library, XLIFF, episodic `/api/*` |
...
- [ ] Mount static trees: `continuuuum/library/`, `Scripts/continuuuum_api/static/`
- [ ] Unified nav: library ↔ lemma-library ↔ episodic hub
- [ ] Remove `lemmaApiBase` / `continuuuumLibraryBase` localStorage from SPAs
- [ ] Point Unity `ContinuuuumLibraryWindow` default URL to merged server (not `:3000` Cave)
...
| Lemma library + XLIFF | `python -m continuuuum_api.server` | `http://127.0.0.1:5050/lemma-library` (different process — use alternate port for one) |

Set `#lemma-api-base` in library toolbar to Drawer 2 API base when running split (e.g. library on 5050, continuuuum_api on 5051).
