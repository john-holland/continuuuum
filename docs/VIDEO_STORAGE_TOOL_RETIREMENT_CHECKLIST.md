...
- [x] Continuuuum endpoints implement every target API in the matrix.
- [x] USC provides all required core media primitives from the contract doc.
- [x] Bit-exact and lossy quality parity tests pass against USC+Continuuuum.
- [x] Stream cache mode (including budget + LRU) is validated in endpoint tests.
- [x] Settings parity is validated (all media knobs exposed in Continuuuum API).
- [x] Docs/reference workflows point to USC+Continuuuum, not `video_storage_tool`.
