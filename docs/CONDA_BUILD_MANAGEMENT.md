...
Conda/Miniconda is used to manage multiple Python versions across **continuuuum**, **unified-semantic-compressor** (USC), and related projects.
...
|-------------|--------|---------|
| `continuuuum` | 3.12 | Continuuuum library server, Entropythief ring, serve_library |
...
Continuuuum depends on USC. Two options:

**Option A: USC in same conda env**
```powershell
cd C:\Users\John\continuuuum
conda env create -f conda-environment.yml
conda activate continuuuum
...
conda activate continuuuum          # Switch to continuuuum
conda activate usc                # Switch to USC
conda deactivate                  # Return to base
conda env remove -n continuuuum     # Remove env
```

## Why Conda?

- **Multiple Python versions**: continuuuum can stay on 3.14 for non-USC work; USC uses 3.12 for torch.
