...
# Build WebGL Continuum Library UI from Drawer 2 into library/continuum_editor_webgl/.
# Env:
...
#   CONTINUUM_WEBGL_OUT Output folder
...
$Out = if ($env:CONTINUUM_WEBGL_OUT) { $env:CONTINUUM_WEBGL_OUT } else { Join-Path $RepoRoot "library\continuum_editor_webgl" }
