...
# Build WebGL Continuum Library UI from the Drawer 2 Unity project into library/continuum_editor_webgl/.
#
# Requires:
...
#   CONTINUUM_WEBGL_OUT — Output directory (default: this repo's library/continuum_editor_webgl)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"

DRAWER2="${DRAWER2_PROJECT:-$(cd "${REPO_ROOT}/.." && pwd)/Drawer 2}"
OUT="${CONTINUUM_WEBGL_OUT:-${REPO_ROOT}/library/continuum_editor_webgl}"
...
"${UNITY_EDITOR_PATH}" -batchmode -nographics -quit \
  -projectPath "${DRAWER2}" \
  -executeMethod BuildContinuumWebGL.BuildFromCli \
  -continuumWebGlOut "${OUT}"
